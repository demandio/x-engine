...
**Mode:** Fully automated (no human approval required)

**Output:** 10-15 scored reply targets with drafted replies, delivered to Slack #x-engine-channel (C0AGXSW1X8A) as threaded messages.
...
Format output using `templates/reply-targets-daily.md`. Save to `output/reply-brief-YYYY-MM-DD.md`. Deliver to Slack #x-engine-channel (C0AGXSW1X8A).

---

## Delivery Spec

### Slack Delivery (Thread-Based to #x-engine-channel C0AGXSW1X8A)

**Parent message** (use `slack_post_message` to channel C0AGXSW1X8A):
- Date, number of targets surfaced, total candidates, kill rate
- Top 3 priority targets with one-line description each
- Upload the full reply-brief .md file as an attachment using `slack_files_upload`

**Thread replies** (use `slack_reply_to_thread` with the parent message's `thread_ts`):
- Post one thread reply per target
- Each reply includes: score, post URL, author, original post excerpt (first 100 chars), suggested reply text, pattern used, gift type
- Keep each thread reply scannable - Mike reads these from his phone

### File Output

Full daily brief using the `templates/reply-targets-daily.md` template. Two-part structure:
- **Part 1: The Scouting** - Full provenance (all candidates, all scores, all kills)
...
The Reply Engine runs daily via cron at 6 AM PST, or manually:
...
5. **Delivery** saves the brief to `output/` and posts to Slack #x-engine-channel (C0AGXSW1X8A) as threaded messages.

If any source is unavailable, log the gap and continue with available sources. Do not halt for human input.

## Parallel Execution

Use subagents (the Agent tool) to parallelize independent work within each stage:

- **Stage 1 (Scout):** Run Slack channel scans in parallel. Run X Twitter MCP search queries in parallel batches.
- **Stage 2 (Score):** Score candidates in parallel batches of 5-8.
- **Stage 3 (Draft):** Draft replies in parallel for all surviving targets simultaneously.
- **Stage 4 (Deliver):** Save file and post to Slack can run in parallel.
