...
**Thematic connection:** If `output/thematic-context.md` exists, check whether this message's Now/Near/Far maps to an active thematic current. If it does, note the connection. This helps Mike see which themes are generating the most content naturally.
...
If any gate fails, apply the suggested fixes and re-run. If the post cannot pass after one revision, flag in the output for manual review.
...
When in doubt, omit. Better to flag a false positive than to leak something.
