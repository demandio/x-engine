...
**Action:** Flag for Dakota before posting. Dakota reviews the framing - not the insight (which already passed scoring) but whether the tone, word choice, and implications are calibrated correctly. Dakota may:
- Approve as-is
...
**Action:** Skip by default. Note in the brief: "RED FLAG: [topic]. Skipped per default protocol. If Mike wants to engage, Dakota will review framing." If Mike or Dakota explicitly requests engagement, treat as Yellow (Dakota reviews framing before posting).
...
**Drafter (Stage 3):** For YELLOW targets, draft the reply but add a "Framing Note for Dakota" section explaining what's sensitive and what framing choices were made. For RED targets, do not draft unless Dakota explicitly includes them.

**Delivery:** In the brief output, group targets by tier. GREEN targets are ready for Mike. YELLOW targets need Dakota's sign-off on framing. RED targets are listed in a separate section with the skip note.
...
**Transform:** For YELLOW posts, the transform output includes a "Sensitivity Framing" section explaining what's sensitive and how the framing addresses it. For RED posts, flag for Dakota and do not transform unless explicitly requested.
